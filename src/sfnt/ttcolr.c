...
typedef struct {
  ...
  Colr colr;
  Cpal cpal;
  /* The memory that's backing colr/cpal tables */
  void* colr_table;
  void* cpal_table;
} ColrCpal;
...
#define FT_COMPONENT  trace_ttcolrcpal
                     ...
                     FT_Stream  stream )
  {
    ...
    if ( table_size < sizeof(ColrHeaderSize) )
      goto InvalidTable;
    ...
    base_glyph_end = base_glyph_begin + colr.num_base_glyphs * BaseGlyphSize;
    layer_end = layer_begin + colr.num_layers * LayerSize;
    if ( base_glyph_end < base_glyph_begin || base_glyph_end > table_size ||
         layer_end < layer_begin           || layer_end > table_size )
      goto InvalidTable;

    /* Ensure pointer don't wrap */
    ...
    if ( table_size < CpalV0HeaderBaseSize )
      goto InvalidTable;
    ...
    colors_end = colors_begin + cpal.num_colors * ColorSize;
    /* Ensure variable length tables lies within the COLR table.      */
    /* We wrap around FT_ULong at most once since count is FT_UShort. */
  ...
  tt_face_free_colr( TT_Face  face )
  {
    ...
    ColrCpal *colr_and_cpal = face->colr_and_cpal;
    if ( colr_and_cpal )
    {
                          ...
                          BaseGlyphRecord*  record )
  {
    ...
    while ( min <= max )
    {
      ...
      FT_Byte  *p    = base_glyph_begin + mid * BaseGlyphSize;
 ...
 Error:
                      ...
                      FT_Byte*   alpha )
  {
    ...
    p = cpal->colors + color_offset + ColorSize * color_index;
    *red = FT_NEXT_BYTE( p );
                            ...
                            FT_GlyphSlot  srcSlot )
  {
    ...
    if ( !dstSlot->bitmap.buffer )
    {
      ...
      memset ( dstSlot->bitmap.buffer, 0, size );
    }
    else
    {
           ...
           y_max != dstSlot->bitmap_top )
      {
        ...
        q = buf + pitch * (y_max - dstSlot->bitmap_top) +
                4 * (dstSlot->bitmap_left - x_min);
        for ( y = 0; y < dstSlot->bitmap.rows; y++)
        {
          memcpy (q, p, dstSlot->bitmap.width * 4);
    ...
    /* Default assign to appease compiler */
    r = g = b = 0;
    alpha = 255;
